...
export function buildAmbientDetails(b: NanaBuilder): void {
  for (const v of nanaVenues) {
    ...
    for (const z of [v.z - 3, v.z + 3]) {
      b.prop('frontage-post', [0.15, 4.5, 0.15], [v.side * 16.2, y + 2.25, z], '#ae705b');
      b.prop('gallery-planter', [0.8, 0.6, 0.8], [v.side * 17, y + 0.3, z], '#766654');
  ...
  for (let i = 0; i < 7; i++) {
    const x = i % 2 ? 4 : -4,
      z = 14 + i * 5;
    b.prop('beer-garden-table', [2.2, 0.1, 1.2], [x, 0.95, z], '#996e51');
    b.prop('table-leg', [0.18, 0.9, 0.18], [x, 0.45, z], '#4d3f42');
    for (const dx of [-1.7, 1.7])
      b.prop('plastic-chair', [0.65, 0.6, 0.65], [x + dx, 0.3, z], i % 2 ? '#895778' : '#558989');
  ...
  b.prop('lift-shaft', [2, 13, 2], [11, 6.5, 64], '#55586c');
  b.sign('LIFT / SERVICE ONLY', [11, 2.5, 62.9], 5, '#dfb55e');
  for (const x of [-7, 7]) {
    b.prop('traffic-signal', [0.4, 2.8, 0.4], [x, 1.4, -45], '#444958');
